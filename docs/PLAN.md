...
- `learning-agent`、`skills-agent`、`plugin-builder-agent` 和 `code-agent` 按需拉起。
- 设计目标中 Level 2 智能体共 4 个，Level 3 智能体共 1 个。
...
- Safety/Permission 第一版是确定性规则模块，不调用 LLM。
- Brain 每轮最终回复前必经审核。
- 控制台用户可见输出默认中文。
- 测试模式必须支持全链路 LLM 接管。
...
| [设计文档/01-参考项目启发.md](./设计文档/01-参考项目启发.md) | Hermes、Mercury、OpenClaw/opencode 的主参考，ClaudeCode 的 Claude Code 编排专项参考，以及 UltraCode、Proma、GSD-2 的限定辅助参考 |
| [设计文档/02-架构设计.md](./设计文档/02-架构设计.md) | 多 Agent 架构、模块解耦、目录结构、数据流、SQLite schema |
| [设计文档/03-能力扩展与自进化.md](./设计文档/03-能力扩展与自进化.md) | Skill、插件系统、自进化流程 |
| [设计文档/04-CLI测试与可观测.md](./设计文档/04-CLI测试与可观测.md) | CLI、LLM 接管、日志、控制台 I/O、CI artifact |
| [设计文档/05-安全类型与多智能体.md](./设计文档/05-安全类型与多智能体.md) | 安全设计、关键类型、三级智能体编排 |
| ~~设计文档/06-实施路线.md~~（已归档） | Phase 0-7 实施计划，已被 [11-架构全景-5.0.md](./设计文档/11-架构全景-5.0.md) 取代 |
| [设计文档/07-开发调试与验证.md](./设计文档/07-开发调试与验证.md) | 依赖、测试模型配置、开发命令、验证计划 |
...
- 详细设计写入 `设计文档/` 对应文件。
- 本文件只维护总览、索引和关键决策。
- 修改开发约束时同步检查 `docs/AGENTS.md` 和 `CLAUDE.md`。

...
> ⚠️ **2026-08-22：1.0 重置起草中**。版本号体系已翻转：旧体系统一 `old-v` 前缀（old-v2 代码线 + old-v15~18 架构升级），**新设计从 1.0 重新计数**。`设计文档/` 分四类：`01-规范/`（已拍板决策）、`02-计划/`（实施路线）、`03-参考/`（外部对标分析）、`废弃/`（old-v 系列过程稿归档，逐篇摘要见 `废弃/README.md`）。当前主线 = [`设计文档/02-计划/1.0-重置路线图.md`](../设计文档/02-计划/1.0-重置路线图.md)。

**版本现状（诚实陈述）**：
- **当前运行代码 = old-v2**：old-v15（Brain 中心化治理 + 存储加固）+ 22（对话内联统一 Block[]）。下文「核心目标」「当前关键决策」描述的就是这套**仍在运行的代码**，开发规则照常适用；其设计过程稿（21 / 22 等）已迁至 `废弃/`。
- **1.0 = 重置起草（规范已拍板）**：推翻 old-v2 / old-v17 / berryagentOS 的设计方向，从白纸重新设计。蓝本读码综合 + 四篇规范（内核边界 / 插件契约 / 运行时骨架 / 会话与存储）已拍板，尚未落码；核心命题 / 技术栈 / 记忆与自进化待起草。
- **old-v17（滚动计划循环）与 old-v18（重置）设计稿均已归档**，结论不延续。
...
- Safety/Permission 三级模型（old-v15）：L1 确定性规则 → L2 Brain 语义判断 → L3 用户确认；YOLO 模式全交 Brain。
- Brain 每轮最终回复前必经审核。
- 控制台用户可见输出默认中文。
- 测试模式必须支持全链路 LLM 接管。

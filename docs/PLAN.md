...
**当前架构标准（已落地）**：[15.0](../设计文档/21-架构升级-15.0.md)（Brain 中心化治理：权限三级审批 / BrainEscalation / Auditor 后置审计 / brain.command 指挥通道 + 存储层加固：FTS5 双索引 / 写竞争 jitter / redact 关卡 / WAL checkpoint / 声明式 schema 演进）+ [22 对话内联统一](../设计文档/22-对话内联统一.md)（工具/委派/MCP 嵌入对话流，`messages`+`message_blocks` 单一时间线）。

**前进设计线（未落地）**：[17.0](../设计文档/25-架构升级-17.0-滚动计划循环.md)（滚动计划循环 — provider / harness+loop / delegate，设计正式稿）+ [18.0](../设计文档/27-架构升级-18.0-berryagentOS概念定位.md)（berryagentOS 概念定位 — 通用超级智能体操作系统，概念稿）。两者建立在 15.0+22 之上，不推翻已落地决策。

核心设计文档（常青，不随版本归档）：

| 文档 | 内容 |
|---|---|
| [设计文档/00-需求与愿景.md](../设计文档/00-需求与愿景.md) | 原始需求、需求拆解、命名规范、项目愿景 |
| [设计文档/02-架构设计.md](../设计文档/02-架构设计.md) | 多 Agent 架构、模块解耦、目录结构、数据流、SQLite schema |
| [设计文档/05-安全类型与多智能体.md](../设计文档/05-安全类型与多智能体.md) | 安全设计、关键类型、三级智能体编排 |
| [设计文档/12-技术栈参考.md](../设计文档/12-技术栈参考.md) | 后端/前端技术栈、版本号、选型理由、备选方案 |
| [设计文档/21-架构升级-15.0.md](../设计文档/21-架构升级-15.0.md) | 当前架构标准（15.0 双支柱） |
| [设计文档/22-对话内联统一.md](../设计文档/22-对话内联统一.md) | 当前架构标准（对话呈现层，Block[] 内联） |

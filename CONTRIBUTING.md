...
> ⚠️ **2026-08-23：src/ 已直接换防 1.0**（第十批拍板 #19）——old-v2 代码已退役进 git 历史，本文描述的 old-v2 开发流程**整体过期**（目录结构/命令/测试均不再对应现状）。1.0 命令：`npm run dev` / `npm test` / `npm run typecheck` / `npm run lint:topology`；设计依据 = [`设计文档/01-规范/`](设计文档/01-规范/)。本文将随 M1 成熟按第九批「全废弃重写」口径重写。

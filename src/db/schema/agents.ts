...
export const agents = sqliteTable('agents', {
  ...
  customEnv: text('custom_env', { mode: 'json' }).$type<Record<string, string> | null>(),
  customArgs: text('custom_args', { mode: 'json' }).$type<string[] | null>(),
  l2Capabilities: text('l2_capabilities', { mode: 'json' }).$type<string[]>().notNull().default(['learning','skills']),
  roles: text('roles', { mode: 'json' }).$type<string[] | null>(),

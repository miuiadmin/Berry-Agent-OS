...
import { getDb, deleteSession } from '../memory/index.js';
...
export function createApiRouter(deps: WebServerDependencies) {
  ...
  route('DELETE', '/conversations/:sid', (_req, res, _url, params) => {
    // 先中断该 session 的活跃 turn（含委派/daemon 子进程 + pending）——防 agent 完成 task.result 时
    // 写已删 session 重建会话（"删了又回来"根因之一）。handleInterrupt → interruptSession（中断活跃委派 + fail pending）
    deps.handleInterrupt(params.sid, '会话已删除');

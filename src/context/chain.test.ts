...
describe('caller 链（导入者归因取数口）', () => {
  it('包裹内可读、包裹外 undefined（host 兜底约定由读点承担）', () => {
    ...
    const inside = runInCallerChain('plugin-a', () => chainCaller());
    expect(inside).toBe('plugin-a');
    expect(chainCaller()).toBeUndefined(); // 包裹结束即出链
  });

  it('异步下游继承：await 之后仍读到身份（服务面跨 tick 调用的形态）', async () => {
    const readAfterTick = runInCallerChain('plugin-b', async () => {
      await Promise.resolve(); // 让出微任务队列后再读
      return chainCaller();
    });
    expect(await readAfterTick).toBe('plugin-b');
  });
  ...
  it('与会话链正交：caller 包裹不染会话链，会话包裹不染 caller 链', () => {
    const both = runInSessionChain({ sessionId: 's1' }, () =>
      runInCallerChain('plugin-c', () => ({ caller: chainCaller(), session: chainSessionId() })),
    );
    expect(both).toEqual({ caller: 'plugin-c', session: 's1' });
    // 单独会话链内 caller 仍无身份

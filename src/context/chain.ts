 ...
 *
 * 写点（唯一两类）：
 * 1. 驱动 launch 边界包裹本驱动 sessionId（runTurns 链上的一切——工具执行/管道/
 *    context_transform 桥/事件落账——都自然继承）；
 ...
 */
export function runInSessionChain<T>(sessionId: string, fn: () => T): T {
  return sessionChain.run({ sessionId }, fn);
}

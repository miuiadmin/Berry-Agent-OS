 ...
 *
 * Disposer 纪律与 tools.register 同款：返回裸 Disposer，应用侧书写
 * `ctx.effect(() => ctx.prompts.registerSection(...))`——作用域 LIFO 回卷即注销段
 * （装载锚 dispose → 段集变化 → prompts_change 广播 → 装配层重建）。
 *
 * 双入口（exec 纵切拆分，骨架篇 §7.3）：装载面 registerSection 须恰含一个
...
/**
 * 段 id 形状·装载面：小写段式路径，须恰含一个 `/`（应用域前缀/段名——
 ...
 * 装载面注册无 `/` id 一律 PROMPT_SECTION_INVALID；宿主半边 API 不经
 * ctx.prompts 装载面，即本对双入口正则拆分的意义）。
 */
const HOST_SECTION_ID_FORMAT = /^[a-z][a-z0-9-]*$/;

/**
 * 宿主段注册通道（组合根 boot 装配期专用——environment 披露段即首例）。
 * 刻意不进 PromptsService 装载面：经 ctx.get('prompts') 拿到的服务对象上
 * 没有本方法，宿主自留地词汇对应用不可达。
 */
export interface HostPromptsRegistry {
  /** 注册宿主自留地段（id 无 `/`；与应用段同表同字典序物化） */
...
export function registerPromptsService(ctx: Context): { service: PromptsService; host: HostPromptsRegistry } {
  ...
  const addSection = (section: PromptSection, gate: 'app' | 'host'): Disposer => {
    ...
    if (gate === 'app' && !SECTION_ID_FORMAT.test(section.id)) {
      throw new AppError(
        PROMPT_SECTION_INVALID,
        `提示词段 id 非法（${section.id}）——须小写且恰含一个 /（应用域前缀/段名，如 memory/core；无 / 的单段 id 是宿主自留地，应用不可注册；契约篇 §1.3 落码形态①）`,
      );
    }
    if (gate === 'host' && !HOST_SECTION_ID_FORMAT.test(section.id)) {
      throw new AppError(
        PROMPT_SECTION_INVALID,
        `宿主提示词段 id 非法（${section.id}）——须小写单段且不含 /（含 / 的域前缀形属应用词汇面）`,
      );
    }
    ...
    materialize(sessionId) {
      ...
      for (const section of ordered) {
        try {
          ...
          // 应用 bug 不杀重建：诊断占位进提示词（模型可见 + 可排查），错误细节进日志
  ...
  // 宿主半边通道：与应用段同表，但词汇面独立（无 `/` 单段 id 专属宿主）

...
describe('S5 审批守门归属（fresh 作用域三件 + answerer 标签 + bash 域注册）', () => {
  ...
  it('双驱动 bash 升权各落各账：answerer 弹窗带 [chat·短id] 标签、approval 对落各自会话（F1+F2+迁域三合一）', async () => {
    ...
    // 弹窗标签：[berrycode·A 短id]（组装批：boot 缺省驱动 = 默认应用 berrycode 域）+
    // #approvalId 短形 + summary + 理由 + 批准问句
    expect(prompts[0]).toContain(`[berrycode·${aId.slice(0, 8)}]`);
    ...
    expect(prompts[1]).toContain(`[berrycode·${bId.slice(0, 8)}]`); // B 同走默认位（berrycode 域）
    expect(prompts[1]).toContain('B 的升权理由');
    expect(prompts[1]).not.toContain(`[berrycode·${aId.slice(0, 8)}]`);

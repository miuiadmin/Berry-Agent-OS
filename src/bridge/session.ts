...
/**
 * 桥面服务调用缺省预算（毫秒）——svc-invoke 两代理腿（宿主调域内服务 /
 * 域内调宿主服务）共享的到点结算上限（契约篇 §1.7 第十一轮遗漏大扫 20260904-b
 * 增补第 1 条）。挂起的服务实现不冻结域事件循环（心跳 ping 照答），无预算即
 * 对端 Promise 永挂——敌意应用可借挂在宿主侧挂死调用方。与 loadTimeoutMs /
 * 工具管道 defaultTimeoutMs 同族缺省；域 spawn 参数可注入覆写（测试专用面：
 * 60s 缺省在测试形态不可观察）。
 */
export const SVC_PROXY_TIMEOUT_MS = 60_000;

...
export interface CompositionRow {
  ...
  runtime?: 'main' | 'worker';
  /**
   * 挂载目标键（契约篇 §5.1 挂载目标两档，D1 清单投影批 2026-08-27 收键）：
   * 缺省 = **系统组合**（母体 = OS——官方件档位，注册面落全局层、进一切组成面）；
   * `<应用id>` = **应用组合**（母体 = 该应用——第三方能力件档位，注册面落该
   * 应用域层、只进该应用组成面）。取值域 = 在册应用清单 id，合成期四触发拒绝式
   * 执法（loadComposition）：未知应用 id / Ring 1 必备行带 app / 官方引用行带
   * app（判源 = 行引用形：`builtin:` 前缀或省略沿用官方层——官方件母体恒系统）；
   * 第四触发「第三方行缺省挂系统拒」随 D2 装机两态同批落（先行落码会把既有
   * install 产物〔无 app 行〕全数拒之门外——装机两态是它的前提）。
   */
  app?: string;
}

/**
 * 行挂载目标投影探针（契约篇 §5.1 D1 清单投影批，2026-08-27）：rowId → appId
 * 的活查询面。组合根构造并维护（boot 与 /reload 各自从组合树重建投影——闭包读
 * 活视图，服务构造时点无关），注入三个注册面服务做 D1 注册面路由：
 * - tools：无显式键注册经 caller 链取行 id → 查本面 → 命中即路由应用域层；
...
export interface RowAppProbe {
  /** 行 id → 挂载应用 id（行无 app 键 / 查无此行返回 undefined） */
  get(rowId: string): string | undefined;
  /** 组合树携带 app 键的行数（warn 门限探测——0 = 零应用行） */
  size(): number;
}

...
import { createAppSqliteFace } from './app-sqlite.js';

/** 临时目录（realpath 归一——macOS /var 与 /private/var 差异教训） */
function makeTempDir(): string {
  return realpathSync(mkdtempSync(join(realpathSync(tmpdir()), 'persist-app-sqlite-')));
}

describe('createAppSqliteFace（第六键注入物工厂）', () => {
  it('主库路径命中即拒：字面相等 / 相对路径 resolve 后相等 / symlink 别名归一后相等 → APP_MAIN_DB_FORBIDDEN', () => {
    const dir = makeTempDir();
    const mainDb = join(dir, 'main.db');
    writeFileSync(mainDb, '');

    const face = createAppSqliteFace(mainDb);
    // 字面相等
    ...
    try {
      ...
      expect((err as AppError).code).toBe(APP_MAIN_DB_FORBIDDEN);
  ...
  it("':memory:' 恒放行（不落盘无碰撞可能），且主库为 ':memory:' 的装配形态不误伤应用自管内存库", () => {
    // 主库是文件路径：应用内存库照开
    const dir = makeTempDir();
    const face = createAppSqliteFace(join(dir, 'main.db'));
    ...
    // 主库本身是 :memory:（测试/:memory: 装配）：应用内存库不误伤、文件路径不撞名
    const memMainFace = createAppSqliteFace(':memory:');
  ...
  it('树外自管库真开库：better-sqlite3 全量实例（DDL/DML 可用），readonly 选项透传', () => {
    const dir = makeTempDir();
    const face = createAppSqliteFace(join(dir, 'host.db'));
    // 应用自己的数据子目录（ctx.paths.appDataDir 形态）
    const appDir = join(dir, 'apps', 'my-plugin');
    mkdirSync(appDir, { recursive: true });
    const db = face.openDatabase(join(appDir, 'own.db'));

 ...
 * 2026-08-26 挖矿批 P0-2）。
 *
 * 插件自管持久化的正路（对比三家生态读码反模式「直读宿主私有库」）：宿主
 * **同实例** better-sqlite3 + 主库路径 fail-loud 拒开——插件拿到的 openDatabase
...
/** 第六键面：插件经 `berryagent/sqlite` 虚拟面取到的对象形（openDatabase 即其成员） */
export interface PluginSqliteFace {
  /**
   * 打开插件自管库（better-sqlite3 全量 Database 实例——自管库不强制 readonly，
   * 读写权限是插件自己的领地；主库路径命中即抛 PLUGIN_MAIN_DB_FORBIDDEN）。
 ...
 */
export function createPluginSqliteFace(mainDbPath: string): PluginSqliteFace {
  // 内存主库（测试/:memory: 装配）无文件身份——基准为字面 ':memory:'，
  // 只拦文件路径撞名；插件开 :memory: 在下方恒放行分支，逻辑自洽
  ...
  return {
    openDatabase(path: string, options?: Database.Options): Database.Database {
      ...
      if (target === mainAbs) {
        throw new AppError(
          PLUGIN_MAIN_DB_FORBIDDEN,
          `拒开宿主主库（${mainAbs}）——会话/凭证/模型目录是宿主私有面；` +
            `插件自管库请用自身数据目录内的路径（ctx.paths.pluginDataDir），或 ':memory:'`,
        );
      }

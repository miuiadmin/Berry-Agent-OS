/**
 * 13.0 §13.10 + §13.16: 长时间任务进度心跳管理器 + maxTaskDuration 自动终止。
 *
 * 解决的问题：
 *   - 长时间任务（>5min）没有 tool_call 输出时，前端看不到「还在工作」的信号
 *   - 任务卡住时无法及时发现
 *   - 任务跑太久（超过 maxTaskDuration）时自动终止
 *
 * 工作机制：
 ...
 *   - 任务完成时自动从追踪中移除
 *   - §13.16: 如果任务运行时间超过 maxTaskDuration，自动 fail delegation
 *
 * 性能：
...
export interface HeartbeatConfig {
  ...
  /** 无活动超过此值才发心跳（ms） */
  thresholdMs: number;
  /** §13.16: 任务最大运行时长（ms），超过自动终止。默认 30 分钟 */
...
const DEFAULT_CONFIG: HeartbeatConfig = {
  ...
  maxTaskDurationMs: 30 * 60 * 1000, // §13.16: 30 分钟自动终止
};
...
export interface HeartbeatSource {
  ...
  /**
   * §13.16: 终止超过 maxTaskDuration 的 delegation。
   * 由 DelegationManager 实现，调用 fail() 方法。
   * @returns true 如果成功终止
   */
...
export class TaskHeartbeatManager {
  ...
  start(): void {
    ...
    logger.info({
      ...
      maxTaskDurationMs: this.config.maxTaskDurationMs,
  ...
  tick(): void {
    ...
    for (const entry of active) {
      ...
      // §13.16: 检查 maxTaskDuration — 超时自动终止
      if (elapsed >= this.config.maxTaskDurationMs) {
        const reason = `任务超时自动终止：运行 ${(elapsed / 60_000).toFixed(1)} 分钟，超过上限 ${(this.config.maxTaskDurationMs / 60_000).toFixed(0)} 分钟`;
        const terminated = this.source.timeoutDelegation(entry.delegationId, reason);
        if (terminated) {
          this.eventBus.emit('task.timeout', {
            taskId: entry.taskId,
            targetAgent: entry.agentName,
          ...
          logger.warn({
            ...
            elapsedMs: elapsed,
          }, 'task-heartbeat: delegation auto-terminated (maxTaskDuration exceeded)');
        }
        continue; // 已终止，跳过心跳
      }

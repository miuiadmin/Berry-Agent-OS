...
describe('AuditRecorder', () => {
  ...
  describe('recordAutoApprove', () => {
    it('A 级 auto-approve 落库 verdict=auto_approve_A_level', () => {
      recorder.recordAutoApprove({
        sessionId: 'ses-A',
        level: 'A',
        draft: 'hi 简答',
        userMessage: '你好',
      ...
      const rows = db.prepare('SELECT * FROM review_requests').all() as any[];
      expect(rows).toHaveLength(1);
      expect(rows[0].session_id).toBe('ses-A');
      expect(rows[0].level).toBe('A');
      expect(rows[0].verdict).toBe('auto_approve_A_level');
      expect(rows[0].draft_response).toBe('hi 简答');
      expect(rows[0].final_response).toBe('hi 简答');
    });

    it('no_intent_anchor auto-approve 落库 verdict=auto_approve_no_intent', () => {
      recorder.recordAutoApprove({
        sessionId: 'ses-N',
        level: 'no_intent_anchor',
        draft: '兜底回复',
        userMessage: '随机消息',
      });
      ...
      const rows = db.prepare('SELECT * FROM review_requests').all() as any[];
      expect(rows).toHaveLength(1);
      expect(rows[0].verdict).toBe('auto_approve_no_intent');
      expect(rows[0].level).toBe('A');
    });

    it('保留 toolCalls 作为审计证据', () => {
      recorder.recordAutoApprove({
        sessionId: 'ses-T',
        level: 'A',
        draft: 'd',
        userMessage: 'u',
        toolCalls: [{ name: 'read_file', input: '/tmp/x', result: 'contents' }],
      });

      const rows = db.prepare('SELECT * FROM review_requests').all() as any[];
      const reviewInput = JSON.parse(rows[0].review_input);
      expect(reviewInput.user_message).toBe('u');
      expect(reviewInput.tool_calls).toHaveLength(1);
      expect(reviewInput.tool_calls[0].name).toBe('read_file');
    });
  });
});

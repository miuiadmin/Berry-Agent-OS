...
Berry Agent OS 的固定内核只司**装、跑、守、存**四职能，其余一切以插件形态装在组合树上。装载件分三种产品角色：**被启动的是应用，被调用的是库，被依赖的是后台服务**——连首启进入的对话本身也是一个应用（官方对话应用 `builtin:chat`，默认层首行），不住内核；预装默认应用的终态是**代码智能体应用**（coding 优先——命令执行纵切已落，组装随下一批，落地前由对话应用兼任默认入口）。双重自进化（使用进化 + 技能进化）走涌现路径：不设中枢调度器，内核只提供原语。应用面机制（应用清单声明、启动面、多应用并行、应用级授权与预算）已落码。装载关系走**应用中心模型**（规范已定、分四刀落码中）：应用是独立安装物（npm 三源即市场，不自建商店）；插件是通用件，**独立不生效**——装机只入仓库、挂载到应用或系统才生效；官方件挂系统组合服务所有应用（记忆等操作者状态长在同一份上），第三方件挂应用组合（授权与爆炸半径随宿主应用收拢）。**下限目标：出厂默认层（官方全家桶）达到 Codex / Claude Code 的可日用水准**——检索、命令执行、联网、MCP 四纵切已落，待代码智能体应用上默认入口后即达。
...
- **插件式增强（Ring 2）**：官方全家桶随包默认装配——`chat`（对话应用——首启对话体验的载体，默认层首行）、`memory`（记忆库：提取/合并/双路注入/跨会话检索/效用进化）、`subagent`（进程内子代理委派 + 声明式子代理）、`goal`（长目标状态机 + 预算刹车 + 续跑）、`scheduler`（定时任务 `/tick`——launchd/crontab 注册器，OS 调度器持时、插件不持进程）、`mcp`（MCP 客户端桥——外部工具生态接入）、`web`（fetch 工具 + SSRF 卫生件）、`compaction`（长会话压缩）、`admin`（平台管理面工具）——每一件都可卸载，卸掉核心循环不断。
...
- **组合树装载**：默认层 + `overlay.yaml` 字段级覆写，`/reload` 热重载插件集；应用中心装载面（挂载目标两档 + 装机/挂载两态）规范已定、落码中。

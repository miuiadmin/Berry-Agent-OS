...
- **事件溯源——Agent 的生命线**：append-only 事件日志 + 投影派生；长会话压缩（`compaction`）、工作区快照回退（`checkpoint` /rewind）、会话分叉与收养全部由日志承载。**Agent 的每一天都是 durable 事实。**
- **官方全家桶（Ring 2，件件可卸）**：`coder`（默认代码智能体应用）、`chat`（对话应用）、`memory`（记忆库：提取/合并/双路注入/跨会话检索/效用进化/TTL/版本链——**Agent 在这里变聪明**）、`subagent`（子代理委派）、`goal`（长目标状态机 + 预算刹车 + 挂钟唤醒——**Agent 在这里跨天续跑**）、`scheduler`（`/tick` 定时任务）、`mcp`（MCP 客户端桥）、`lsp`（语言服务器桥）、`web`（fetch + SSRF 卫生件）、`obs`（观测面：聚合 + 告警）、`admin`（平台管理面）、`webui`（回环 Web 通道）。
- **安全栈内建**：工具三段管道（schema 校验 → 守门 → 执行）、沙箱三档、审批对、allowlist 免问。**应用生而沙箱，权限是声明出来的，不是偷来的。**
- **技能系统**：SKILL.md 双层结构 + 渐进披露——**Agent 在这里学会新本事**。
- **组合树装载**：默认层 + `overlay.yaml` 字段级覆写；装机/挂载两态、全局/应用两档、`/reload --app` 单区热重载。
...
给 Berry 写一个应用只需要一个 `index.ts`：默认导出 `apply(ctx, config)`，35 个生命周期钩子横跨会话/代理/轮次/消息/工具管道/provider 六层。详见[应用开发指南](docs/应用开发指南.md)。
...
**默认零遥测**——本工具不发任何网络包：无使用统计、无崩溃上报、无版本检查。你配置的模型调用是唯一的出网流量。**你的 Agent 的生活只属于你。**

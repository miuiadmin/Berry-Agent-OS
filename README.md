...
> 插件式智能体运行时 — TypeScript + SQLite + pi-ai。

Berry 的宿主内核只做一件事：**问 → 做 → 守 → 存** 的最小对话闭环。其余一切——记忆、子代理委派、长目标续跑、技能、更多通道——全部以插件形态挂在内核周围的扩展点上。双重自进化（使用进化 + 技能进化）走涌现路径：不设中枢调度器，内核只提供原语。

...
**28** 模块（全部有码）· **27** 生命周期钩子 · **28** 类 durable 事件 · **15** 件官方全家桶（14 件 Ring 2 + 默认应用 berrycode，件件可卸）· **2,700+** 测试 · **0** 遥测。
...
- **28 模块单向 DAG**：全部有码，`npm run lint:topology` 机器执法——装/跑/守/存四职能之外不设中枢，不可卸载。
...
| [docs/API参考.md](docs/API参考.md)           | 公开 API 全符号面（逐符号一句话语义 + 稳定性分级，生成物） |
| [COMPATIBILITY.md](COMPATIBILITY.md)         | API 兼容性档案（面变更史与废弃登记，生成物）               |

...
/**
 * 知识域指路滤词正则（API 治理进化刀 J——查 10 公开产物指路卫生的单源判据）。
 *
 * 命中即「指路知识域」：八篇规范篇名（含「设计文档」根路径指称、「契约篇」
 * 篇内章节形与「骨架篇」缩写形——刀 L desc harvest 实证缩写漏网补收）+
 * `§6.13` 章节号族（API 治理章专属——公开产物用公开锚不用规范
 * 节号）+ 知识域目录名（02-计划 / 03-参考）。两消费方共享单源：check-api 查 10
 * （公开产物面执法）与 API 参考生成器 desc harvest 滤词（刀 L——防产码 JSDoc
 * 首句里的知识域指路灌进公开文档撞查 10）。产码注释引用规范不受此限（§6.13.8
 * 查 10 面界——本正则只喂机器扫描面，不约束人写注释）。
 */
export const KNOWLEDGE_DOMAIN_RE =
  /(设计文档|契约篇|应用契约与扩展点|核心命题|价值主张|内核与应用边界|运行时骨架|骨架篇|会话与存储|记忆与自进化|技术栈篇|§\s?6\.13|0[23]-(?:计划|参考))/;

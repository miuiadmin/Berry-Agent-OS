  ...
  try {
    ...
    try {
      ...
      await callTool('goal_set', {
        ...
        // 预算裕度必须远大于链实耗：贴近实耗会出现「录制时未刹 / 回放时被刹」的
        // 临界漂移——刹车收尾注入会多发一次计划性模型调用，金样按调用序消费即
        // 错位发散（2026-08-27 回放 19/20 发散的实锤根因）。实耗水位随 goal 批
        // 演进上涨：8-27 约 5 万 → 2026-08-31 goal 刀三/四（预算复验 + 沉淀摘要
        // + 续跑注入多轮）后实测约 21 万（真跑 206951/200000 被刹实证）——预算
        // 400 万留约 20 倍裕度。刹车边界行为由 goal 件单元测试覆盖，冒烟流只验
        // 端到端接线，不压预算边界。

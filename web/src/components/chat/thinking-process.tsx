...
export const ThinkingProcess = memo(function ThinkingProcess({ steps, reasoning, isActive, durationMs }: ThinkingProcessProps) {
  ...
  /*
   * 外层容器故意不加 margin：与上方「时间戳行」、下方「工具调用块」的间距由
   * MessageTimeline 的 spacingForPair 按「相邻 block type pair」决定（紧贴 / 中等 / 原 8px 三档），
   * 不在此处自加 margin，避免与外层规则叠加。
   *
   * 即便相邻块 margin 全为 0，视觉上仍会保留约 2~4px 空隙：这来自 CSS line-height（行高 leading），
   * 而非 margin/padding。text-[11px] 默认 normal 行高约 1.2，行盒高度 ~17px，而字符字身实际占高
   * 仅 ~11px，行盒上下各留 ~3px leading 半距；两行行盒紧贴时，这上下半距叠加即形成可见间距。
   *
